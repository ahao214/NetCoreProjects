...
namespace Payment.WebAPI.Controllers
{
    ...
    public class PaymentController : ControllerBase
    {
        ...
        {
            var lineItems = new List<GoodsDetail>();
            decimal TotalAmount = 0m;
            foreach (var product in payments)
            {
                lineItems.Add(new GoodsDetail
                {
                    //GoodsId = product.productId.ToString(),
                    //AlipayGoodsId = product.productId.ToString(),
            ...
            else
            {
                ...
                //把商品细节存进缓存
                await _cache.SetStringAsync($"goods_{model.OutTradeNo}", JsonSerializer.Serialize(lineItems), new DistributedCacheEntryOptions
                {
