...
namespace Order.WebAPI.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        [HttpGet("{orderId}")]
        //[Authorize]
        public async Task<ActionResult<ServiceResponse<Order.Domain.Entity.OrderItem>>> GetOrderDetail(Guid orderId)
        {
            var result = await _orderRepository.GetOrderDetails(orderId);

            ServiceResponse<List<Order.Domain.Entity.OrderItem>> resp = new ServiceResponse<List<Domain.Entity.OrderItem>>() { Data = result };
            return Ok(resp);
        }

        [HttpGet]
        //[Authorize]
        public async Task<ActionResult<ServiceResponse<List<Domain.Entity.Order>>>> GetOrders()
        {
            var result = await _orderRepository.GetOrders(GetUserId());
            ServiceResponse<List<Domain.Entity.Order>> resp = new ServiceResponse<List<Domain.Entity.Order>>() { Data = result };
            return Ok(resp);
        }
        ...
        public async Task<string> AlipayNotify()
        {
            ...
            if ("TRADE_SUCCESS".Equals(form["trade_status"]))
            {
                ...
                //    //发布消息
                //    await _rabbitMqService.PublishMessage("joker_shop_cart", "cart", CartJson, "RemoveCart", "direct");
                //}
